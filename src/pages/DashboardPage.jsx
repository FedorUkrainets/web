    ...
    <div className="layout">
      ...
      <main className="main-content page-stack">
        <div>
          ...
          <p className="muted">Краткий обзор сети зарядных станций.</p>
        </div>
        ...
        <section className="card">
          <div className="row-between">
            ...
            <button type="button" onClick={() => navigate('/stations')}>Открыть станции</button>
          </div>
              ...
              <tbody>
                {stations.slice(0, 8).map((station) => (
                  <tr key={station.id}>
                    <td>{station.name}</td>
                    <td>{station.code}</td>
                    <td>{STATUS_LABELS[station.status] || station.status}</td>
                    <td>{station.current_load_kw}/{station.capacity_kw} кВт</td>
